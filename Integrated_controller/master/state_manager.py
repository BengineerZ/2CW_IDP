...
class robot_state_manager:
	def __init__(self, master_robot):
		...
		self.task_tree["sweep"] = {
			...
			"condition": lambda env, robot: self.optimum_block(robot, env) != None and self.master_robot.getTime() < 270000
		...
		self.task_tree["return_home"] = {
			"next": "sweep",
			"next_alt": "explore",
			"condition": lambda env, robot: (env.total_blocks_detected >= 8 and env.total_blocks_removed == env.total_blocks_detected) or self.master_robot.getTime() > 27000
		}
	...
	def make_robot_state_from_task(self, env, robot):
		if self.current_task == "start":
			...
			if robot.robot_id == 0:
				...
				return ["go_to_target", {"target": (7, 73), "early_stop": 5, "grip": 1, "block": False, "empty": True, "look_at": False, 'optimise_heading': False}]
			else:
				self.blue_current_target = (7,7)
				return ["go_to_target", {"target": (7, 7), "early_stop": 5, "grip": 1, "block": False, "empty": True, "look_at": False, 'optimise_heading': False}]
			...
			if robot.robot_id == 0:
				...
				self.blue_current_target = [int(env.explore_uncertainty()[1]), int(env.explore_uncertainty()[1])]
	...
	def optimum_block(self, robot, env):
		...
		for block in block_list:
			...
			if not(robot.path_possible([int(block[0]),int(block[1])])):
				...
				combined_heuristic = distance*1 + -other_robot_distance*1 + danger*2 - h(other_target, p)

				if robot.robot_id + block[2] ==2:
					new_block_list.append([block[0],block[1],(combined_heuristic)*0.5])
				elif block[2] ==0:
