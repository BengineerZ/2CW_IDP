...
#	def go_to_target(self, args):
	...
	def straight_line_path(self, obstacle_grid, target):
		''' generates the straight line path to a target
		args: 
	...
	def escape_path(self, obstacle_grid, threshold=10):
		''' generates the Djikstra path to a target
		args: 
			nparray: obstacle_grid
			nparray: target - target location
	...
	def straight_line_follow(self, args, path_temp, speed_factor=1):
		''' follows the straight line path
		args: 
	...
	def go_to_target(self, args):
		...
		other_bot = convert_to_grid_coords(self.robot_data[(self.robot_id+1)%2][1:3])
		rr, cc = ellipse(int(other_bot[0]) , int(other_bot[1]), 14,14, shape=self.driving_grid.shape)
		for i in range(len(rr)):
	...
	def block_extent_routine(self, args):
		''' checks for block extent, drives towards block extent
		args: 
			nparray[2]: target - taget location
	...
	def raw(self, args):
		''' direct raw robot control passthrough
		args: 
