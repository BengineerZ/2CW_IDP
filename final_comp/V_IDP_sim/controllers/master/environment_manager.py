...
def log_odds_ratio(p):
	''' calculate the log-odds ratio of a specific value
	args: 
		float: p - grid space probability
	return:
		float: log odds ratio of p
	'''
	return np.log(p/(1-p))

def inv_log_odds_ratio(l):
	''' calculate the inverse log-odds ratio of a specific value
	args: 
		float: l - grid space log odds ratio
	return:
		float: probability
	'''
...
def update_occupied(grid, coord, p = 0.6):
	''' updates an occupied space in the occupancy grid
	args:
		numpy array: grid - occupancy grid
		numpy array: coord - coordinate to be updated 
		float: p - probability of occupation
	return:
...
def update_free(grid, coord, position, p=0.4):
	''' updates the free space coordinates along a line
	args:
		numpy array: grid - occupancy grid
		numpy array: coord - coordinate to be updated 
		numpy array: position - position of the robot
		float: p - probability of non-occupation
	return:
		numpy array: x position of updates
		numpy array: y position of updates
		numpy array: updated probability array
	'''
...
def block_padding(blocks, occupancy_grid, size=9):
	''' pads the occupancy grid with circles of blocks
	args:
		list: blocks - list of blocks
		numpy array: occupancy_grid
		int: size - size of padding radius
	return:
		numpy array: padded grid
	'''
...
def pad_grid(occupancy_grid, iterations):
	''' pads the occupancy grid via gaussian blur
	args:
		numpy array: occupancy_grid
		int: iterations - number of iteration of padding
	return:
		numpy array: padded grid
	'''
...
class environment_manager:
	...
	def __init__(self):
		''' General environment manager class, must be initialised before updaing
		attributes: None
		'''
	...
	def __call__(self):
		''' environment_manager call - must be run at every timestep
		'''
	...
	def update_binary_occupancy_grid(self, robot_data):
		''' updates the binary occupancy grid using new robot data
		args:
			list[2][7]: robot_data - updated data recieved from the robot
		'''
		max_range = 1.4
		for i in [0,1]:
	...
	def driving_grid(self):
		''' generates a grid of driveable area
		args: 
			None
		returns:
			numpy array: final grid - final grid of driveble area
			numpy array: reduced_grid - final grid of reduced driveble area
		'''
	...
	def remove_block(self, target):
		...
		args: 
			numpy array[2]: target - target coordinate to be removed
		'''
	...
	def combine_coord_sets(self):
		''' Union of current set of coordinates and old set 
		args: 
			None
		'''
	...
	def update_block(self, block_state):
		''' updates the state of a block
		args: 
			list[3]: block state - state of the block
		'''
	...
	def explore_uncertainty(self):
		''' computes the coordinate with which to explore the highest area of uncertainty
		args: 
			None
		returns:
			numpy array[2]: explore_coordinate
		'''
	...
	def optimum_heading(self, target, additional_coords, display=False):
		''' computes the optimum heading with which to approach the block
		args: 
			numpy_array: target - target location
			nparray: additional_coords - any other coords to take into acocund
			booL; display - generate a visualiser grid or not
		returns:
			numpy array[2]: explore_coordinate
		'''
